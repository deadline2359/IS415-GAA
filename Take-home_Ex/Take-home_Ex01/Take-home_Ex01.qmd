...
Looking at Nigeria's state boundary [Dataset](#dataset-1), there are many fields and rows that not necessary for our project. Hence, we will filter for those of Osun, and select only **"ADM1_EN"** and **"ADM2_EN"** fields which hold information on 1st and 2nd level administrative zones.
...
At 95% confident interval, we will set `nsim` to be 39 and reject the null hypothesis if the p-value is small than the alpha value of [**0.05**]{.underline}.
