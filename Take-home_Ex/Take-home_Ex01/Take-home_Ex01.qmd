...
-   **WPdx+** **Dataset (CSV)** - Taken from [WPdx Global Data Repositories](https://www.waterpointdata.org/access-data/). It shows the locations of the water points.

-   **State boundary GIS Datasets of Nigeria** - Taken from [Humanitarian Data Exchange](https://data.humdata.org/). It gives geospatial data of Nigeria, in particular the boundaries of its states and Local Government Areas (LGA).
...
Looking at Nigeria's state boundary [Dataset](#dataset-1), there are many fields and rows that not necessary for our project. Hence, we will filter for those of Osun and select only **"ADM1_EN"** and **"ADM2_EN"** fields which will tell us the 1st and 2nd level administrative zones.
...
Using the [`envelope()`](https://rdrr.io/cran/spatstat/man/envelope.html) of **spatstat** package, **Monte Carlo** simulations have be developed to observe the randomness of the water points.
...
```{r eval=FALSE}
L_functional.csr <- envelope(wp_functional_ppp.km, Lest, nsim = 39, rank = 1, glocal=TRUE)
...
```{r eval=FALSE}
L_nonfunctional.csr <- envelope(wp_nonfunctional_ppp.km, Lest, nsim = 39, rank = 1, glocal=TRUE)
